...
from ..utility.builtin_classes.iterables import split_chunks

from ..utility.general import standardize_padding


def ensure_pil_image(image_or_path):
    """
    ...
    :param image_or_path: A path to an image file or a PIL.Image.Image object
    ...
    elif isinstance(image_or_path, ndarray):
...
def pad_image(pil_image, padding, pad_color=(255, 255, 255)):
    ...
    :param padding: A tuple or list of length 1, 2, or 4, specifying the padding to apply. If the length is 1, the same
                    padding is applied to all sides. If the length is 2, the first value is applied to the left and
                    right sides, and the second value is applied to the top and bottom sides. If the length is 4, the
...
def stitch_image_list(*images, rows=None, columns=None, pad_color=(255, 255, 255)):
    ...
    _row_images = []
    for _, image_row in zip(range(rows), split_chunks(images, columns)):
